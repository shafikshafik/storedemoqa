...
public class ExcelReader {
	...
	public List<String> getRowData(final String sheetName, final int rowNum) {
		System.out.println("row number inside the sheet " + rowNum);
		...
		try {
			...
			if (sheet.getRow(rowNum - 1) != null) {
				for (int i = 0; i < sheet.getRow(rowNum-1).getLastCellNum(); i++) {
					...
					if (cell.getCellType() == Cell.CELL_TYPE_STRING)
						...
						lst.add(String.valueOf(cell.getBooleanCellValue()));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	// message
	public boolean addHyperLink(final String sheetName, final String screenShotColName, final String testCaseName,
			final int index, String url, final String message) {
	...
	public static void main(final String arg[]) throws IOException {
		...
		datatable = new ExcelReader(
				"C:\\Users\\xprk661\\prokarmamiddleware\\Hybrid-Framework\\src\\test\\java\\com\\totsy\\xls\\Suite.xlsx");
